...
function AdminProperties() {
    ...
    const [adminListingsToShow, setAdminListingsToShow] = useState(8); // initial number of listings to show
                    ...
                    <section className="flex flex-col items-center bg-white">
                        <div
                            ...
                            {properties.slice(0, adminListingsToShow).map((property) => (
                                <AdminCard key={property.propertyId} property={property} image={property.image}/>
                            ))}
                        </div>
                        <div className="flex items-center justify-center m-5 mt-10">
                            <Button variant="outlined" className="px-10 text-[1rem] text-[#444445]"
                                    onClick={() => setAdminListingsToShow(adminListingsToShow + 8)}>Show More listings</Button>
                        </div>
                    </section>
                </div>
            </Element>

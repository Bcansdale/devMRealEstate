...
export const createCompleteUser = async (db) => {
    ...
    try {
        ...
        let permission = await db.permission.findOne({
            where: {
                permissionName: {
                    [db.Sequelize.Op.iLike]: "Manage Users", // Case-insensitive match
                },
            },
            transaction,
        });
        ...
        if (!permission) {
            permission = await db.permission.create(
                {
                    permissionName: "Manage Users", // Insert it with proper casing
                    // No permissionId should be provided if permissionId is auto-incrementing
                },
                { transaction }
            );
            console.log("Manage Users permission created because it was not found.");
        } else {
            console.log("Manage Users permission found.");
        }
        ...
        let propertyType = await db.propertyType.findOne({
            where: {
                typeName: {
                    [db.Sequelize.Op.iLike]: "Apartment", // Case-insensitive match
                },
            },
            transaction,
        });
        ...
        if (!propertyType) {
            propertyType = await db.propertyType.create(
                {
                    typeName: "Apartment", // Insert with proper casing
                },
                { transaction }
            );
            console.log("Apartment property type created because it was not found.");
        } else {
            console.log("Apartment property type found.");
        }
